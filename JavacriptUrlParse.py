brands = [
    '1921_10001010_vintage__', 'aquasilk_10111100_silk_overdyed', 
    'ashbury_10101011_moroccan__', 'aura_10111100_silk__', 
    'barabara_10011110_vintage__', 'cablearan_10001001_contemporary_moroccan', 
    'cascade_10001000_contemporary__', 'chimera_10011000_silk__', 
    'cinqueterre_10101000_contemporary__', 'colorreform_10001010_overdyed_vintage', 
    'crpatchwork_10001010_overdyed__', 'crsilk_10001010_silk_overdyed', 
    'crspectrum_10001000_overdyed__', 'dama_10001010_vintage__', 
    'dlb_10001010_antique/vintage__', 'ethos_10111100_silk__', 
    'fez_10011000_flatweave__', 
    'fresco_10001010_vintage__', 'kinetic_10101000_contemporary__', 
    'lumina_10001000_contemporary_silk', 'manzara_10111110_contemporary__', 
    'masana_11101101_flatweave__', 'maza_10001000_flatweave__', 
    'nabati_10011100_flatweave__', 'nandi_11101110_flatweave__', 
    'nuvibrant_10101000_contemporary_silk', 'orion_10001000_contemporary__', 
    'ovas_10001000_contemporary__', 'petra_10001000_flatweave__', 
    'prism_10101000_contemporary_silk', 'rio_10101010_contemporary_silk', 
    'samoke_10001000_classic_silk', 'sunclipse_10111000_silk__', 
    'terra_10001000_contemporary__', 'tesselation_10001000_contemporary__', 
    'thera_10001000_contemporary__', 'tulu_10011111_vintage_moroccan', 
    'umbra_10001000_contemporary__',
...
materials = [
    ...
    'Rayon','Seagrass', 'Shearling', 'Sheepskin', 'Silk', 'Sisal',
...
slide2 = {
    ...
    'sunclipse':'Transcend dimensions in a balance of light, color, and design with the Sunclipse collection. Recycled sari silk is handwoven and washed in India, revealing intricate motifs and designs that harmonize with spontaneous bursts of color. Stylized medallions in each one-of-a-kind rug are evocative of antique 19th century designs, depicting intriguing narratives passed down through generations. Iridescent, extraordinary  foundations, Sunclipse cultivates fascination, beauty, and intrigue \– an ordinary canvas transformed into a magical experience. View all <a href="http://www.abchome.com/shop/sunclipse">Sunclipse</a>.',
    ...
    'wovenlegends':'A renaissance of ancient weaving traditions, Woven Legends celebrates the art of meticulously handmade carpets, echoing a time when cultures and communities centered around the craft. Today, the collection revives the past, eliciting a visceral reaction to an alchemy of color and design. Handwoven from locally sourced, vegetable-dyed wool in Turkey, each carpet translates traditional methods of craftsmanship and ancient symbolism into new timeless works of art. With an amalgam of individual weaving styles and a spectrum of hues, the Woven Legends collection is a meditation on tradition \– redefining artistic expression, artisanal design, and community for the modern world. ',
         }
nonbrands = [
    'Antique_10001000___', 'Classic_10001000_Classic__', 
    'Contemporary_10001000_Contemporary__', 'Flatweave_10001000_Flatweave__', 
    'Moroccan_10001000_Moroccan & Textured__',
                ]

bannedMaterial = ['Viscose', 'Polyester', 'Rayon']

def FindBrandIcon(toPrint,database):
    #Function if there is a brand found
    def SearchBrand(toPrint,database):
        for x in range (0,len(database)):
            data = database[x].split('_')
            if data[0] in toPrint:
                # Brand | Icons | Cat | CMCat
                return [data[0],data[1],data[2],data[3]]
    #Function if there is no brand found
    def UnbrandedConversion(toPrint,database):

        unbrand_list = []
        for x in range (0,len(database)):
            data = database[x].split('_')
            if data[0].lower() in toPrint.lower():
                unbrand_list.append(data[0])
                break

        #Special cases for certain combinations of unbranded keywords
        def SpecialCases(key1,key2):
            if key1 in unbrand_list and key2 in unbrand_list:
                unbrand_list.remove(key1)
                unbrand_list.remove(key2)
                unbrand_list.append(key1+' '+key2)               
        SpecialCases('Moroccan','Flatweave')
        
        if unbrand_list == []:
            return ['','10001000','Vintage','']
        else:
            return [unbrand_list[0],data[1],data[2],data[3]]
        
    if SearchBrand(toPrint,brands) != None:
        return SearchBrand(toPrint,brands)
    else:
        return UnbrandedConversion(toPrint,nonbrands)
def FindSKU(toPrint):
...
def FindMaterial(toPrint,database):
    ...
    try:
        ...
        print ('\nError: Material type not found or not supported for: '+FindSKU(toPrint)[0])
...
def CrossMerch(toPrint):
    if FindBrandIcon(toPrint,brands)[3] != '':
        return 'Carpet & Rugs'
    else:
        return ''
def LocateSlide2(toPrint):
    while True:
        try:
            return slide2[FindBrandIcon(toPrint,brands)[0]]
        except KeyError:
            return ''
def OverviewOutput(toPrint):
    if FindMaterial(toPrint,materials)[0] == 'Wool Sisal':
        return 'This carpet evokes the natural aesthetic of sisal while offering the softness and versatility of wool. The neutral hue creates a beautiful foundation for the home. For custom sizing and binding, please call 646.602.3499 to speak to a broadloom specialist.'
    else:
        return ''  
def BannedWords(toPrint,database):  
    statement = toPrint
    for x in range(0,len(database)):
        if database[x] in statement:
            statement = statement.replace(database[x]+' ','')
            statement = statement.replace(database[x],'')
    return statement   
def SplitByDash(toPrint):
    stack = []
    for x in range(0,len(toPrint)):
        stack.append(toPrint[x].split('_')[0].capitalize())
    return stack
...
def Output(toPrint):
    ...
    BannedWords(FindBrandIcon(toPrint,brands)[0].capitalize(),SplitByDash(nonbrands))+'\t'+	#Brand
    BannedWords(itemtitle_string,bannedMaterial)+'\t'+ 	#Item Title
    BannedWords(displaytitle_string,bannedMaterial)+'\t'+#Display Title
    OverviewOutput(toPrint)+'\t'+						#Overview
    FindBrandIcon(toPrint,brands)[0].capitalize()+'\t'+	#Slide 1
    LocateSlide2(toPrint)+'\t'+							#Slide 2
...
#sample = "1234567_moroccan_india_sisal_wool_6-9x12-9_1000_500.jpg"
#print (SortIcons(FindBrandIcon(sample,brands)[1][7]))
#print FindSKU(sample)
#print FindMaterial(sample,materials)
#print (FindBrandIcon(sample,brands))
#print SortIcons(FindBrandIcon(sample,brands)[1][0])
#Output(sample)


def CleanOutput(toPrint):
    everyInput = toPrint.split('.jpg')
    for x in range(0,len(everyInput)):
        return (everyInput)

user_input = input('Enter your data: ')

x = 0
while len(CleanOutput(user_input)[x]) > 0:
    Output((CleanOutput(user_input)[x]))
    x+=1
